...
const defaultUrl = 'rtmp://bintu-play.nanocosmos.de:80/play';
const defaultServer = {
...
function addEntries () {
    ...
    while ((unparsedEntry = deepCopy(paramsObject['entry' + suffix]))) {
        ...
        if (unparsedEntry.rtmp) {
            extend({
                'rtmp': {
                    'url': defaultUrl
                },
                'server': defaultServer
            }, parsedEntry.h5live, configTypes.source.entries.entry.h5live);
        }
        else if (unparsedEntry.bintu && unparsedEntry.bintu.streamid) {
            parsedEntry.bintu = {
